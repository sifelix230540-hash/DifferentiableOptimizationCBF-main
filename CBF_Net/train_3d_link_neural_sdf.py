...
用法:
    python CBF_Net/train_3d_link_neural_sdf.py \
        --urdf assets/robots/9_axis/urdf/9_axis.urdf \
...
def train_link_sdf_unsupervised(
    ...
    *,
    ...
    div_decay_params: tuple[float, ...] = (0.0, 0.5, 0.75),
    div_type: str = "dir_l1",
    ...
    surf_pts = torch.tensor(dataset.surface_points, dtype=torch.float32, device=device)
    surf_normals = torch.tensor(dataset.surface_normals, dtype=torch.float32, device=device)
    n_surf = surf_pts.shape[0]

    # div weight decay state
    decay_params_list: list[tuple[float, float]] | None = None
    ...
    for iteration in range(n_iterations):
        ...
        # 均匀采样 non-manifold 点
        nonmnfld_pts = torch.FloatTensor(n_points, 3).uniform_(
        ...
        # gradients
        mnfld_grad = torch.autograd.grad(
            mnfld_pred.sum(), mnfld_pts,
            create_graph=True, retain_graph=True,
        )[0]
        nonmnfld_grad = torch.autograd.grad(
            nonmnfld_pred.sum(), nonmnfld_pts,
            create_graph=True, retain_graph=True,
        )[0]
        ...
        if w_div > 0:
            if div_type == "dir_l1":
                dot_grad = (nonmnfld_grad * nonmnfld_grad).sum(dim=-1, keepdim=True)
                hvp = 0.5 * torch.autograd.grad(
                    dot_grad, nonmnfld_pts,
                    grad_outputs=torch.ones_like(dot_grad),
                    retain_graph=True, create_graph=True,
                )[0]
                div_val = (nonmnfld_grad * hvp).sum(dim=-1) / (
                    (nonmnfld_grad ** 2).sum(dim=-1) + 1e-5
                )
        ...
        # ----- div weight decay (对齐 StEik 源码) -----
        if decay_params_list is None:
            params = div_decay_params
            decay_params_list = list(zip(
                [params[0], *list(params[1:-1])[1::2], params[-1]],
                [0, *list(params[1:-1])[::2], 1],
            ))
        if div_decay == "linear" and len(decay_params_list) >= 2:
            curr = iteration / n_iterations
            we, e = min(
                [t for t in decay_params_list if t[1] >= curr],
                key=lambda t: t[1],
            )
            w0, s = max(
                [t for t in decay_params_list if t[1] <= curr],
            ...
            if iteration < s * n_iterations:
                w_div = w0
            elif iteration < e * n_iterations:
                w_div = w0 + (we - w0) * (iteration / n_iterations - s) / (e - s)
            else:
                w_div = we

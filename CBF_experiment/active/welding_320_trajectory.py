...
class JointWaypointTrajectory:
    ...
    def __init__(
        ...
        self.planning_diagnostics = {} if planning_diagnostics is None else dict(planning_diagnostics)
        self.joint_waypoints = None if joint_waypoints is None else [np.array(q, dtype=float) for q in joint_waypoints]
...
class PathProgressTrajectory:
    ...
    def advance_progress(self, current_progress: float, projected_progress: float, max_step: float | None = None) -> float:
        advanced = max(current_progress, projected_progress)
        if max_step is not None:
            advanced = min(advanced, current_progress + max(float(max_step), 0.0))
        return float(np.clip(advanced, 0.0, self.progress_end))

"""3_13 版本：在 3_12 基础上加入 MPC-DCBF 控制器。

与 3_12 的差异：
1. 抽象出 Controller 基类，CBFQPController / MPCDCBFController 均为子类；
2. MPCDCBFController 在预测时域内用线性化动力学滚动优化，
   兼顾多步跟踪精度和离散 CBF 安全约束；
3. config.controller_type = "cbf_qp" | "mpc_dcbf" 一键切换。

MPC-DCBF 数学模型
─────────────────
  状态: q̃ = [base_xy, q_joints] ∈ ℝ⁸
  控制: u = [v_base, dq] ∈ ℝ⁸  (速度)
...
class ExperimentConfig:
    ...
    mpc_tracking_weight: float = 15.0
...
class SimulationScene:
    ...
    def _build_environment(self) -> float:
        ...
        bh, dh, wr, ww = (self.config.cart_body_half, self.config.cart_deck_half,
                           self.config.wheel_radius, self.config.wheel_width)
        body_z = wr + bh[2]
        deck_z = wr + 2 * bh[2] + dh[2]
        top = wr + 2 * bh[2] + 2 * dh[2]
        for half, z, rgba in [(bh, body_z, [0.18, 0.22, 0.28, 1]),
                               (dh, deck_z, [0.82, 0.84, 0.88, 1])]:
            ...
            self.cart_parts.append((bid, np.array([0, 0, z], dtype=float), (0, 0, 0, 1)))
        wvis = p.createVisualShape(p.GEOM_CYLINDER, radius=wr, length=ww,
                                   rgbaColor=[0.08]*3+[1], specularColor=[0.2]*3)
        wq = p.getQuaternionFromEuler([math.pi/2, 0, 0])
        wx, wy = bh[0]*0.72, bh[1]+ww*0.35
    ...
    def update_cart_pose(self, base_xy):
        bxy = np.asarray(base_xy, dtype=float).flatten()[:2]
        for bid, lp, lq in self.cart_parts:
            p.resetBasePositionAndOrientation(bid, [bxy[0]+lp[0], bxy[1]+lp[1], lp[2]], lq)

    def _draw_axes(self):
        al = 0.12; o = [0, 0, self.table_height+0.001]
    ...
    def update_status(self, text):
        self.status_text_id = p.addUserDebugText(
            text, [0.02, -0.26, self.table_height+0.52], [0.1]*3, textSize=1.2,
...
class JakaRobot:
    ...
    def __init__(self, config: ExperimentConfig, scene: SimulationScene):
        self.config = config
        self.scene = scene
        self.body_id = p.loadURDF(config.urdf_path, basePosition=[0, 0, scene.table_height],
    ...
    def get_augmented_ee_jacobian(self, q, dq):
        ...
        base = np.vstack([np.array([[1, 0], [0, 1], [0, 0]], dtype=float), np.zeros((3, 2))])
...
class SphereObstacle(Obstacle):
    def __init__(self, cfg, scene):
        ...
        self._init = np.array([x, y, scene.table_height+z])
...
class CBFQPController(Controller):
    ...
    def solve(self, q, dq, ee_pos, ee_quat,
        ...
        dq_nom = np.concatenate([np.zeros(2),
...
class MPCDCBFController(Controller):
    ...
    def __init__(self, robot: JakaRobot, config: ExperimentConfig,
        ...
        single_bnd = ([(-config.base_vel_limit, config.base_vel_limit)] * 2
